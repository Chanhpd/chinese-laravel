...
@section('title', 'HSK ' . $level . ' Vocabulary - Chinese Learner')
...
@push('styles')
    ...
    <link rel="stylesheet" href="{{ asset('client-assets/css/hsk-level.css') }}">
@endpush
...
@section('content')
    <div class="client-container hsk-level-page">
        @include('client.components.header')

        <main class="client-main">
            <!-- Back Button -->
            <div class="mb-4">
                <a href="{{ route('client.hsk.index') }}" class="back-button">
            ...
            <section class="page-header mb-4">
                <div class="text-center">
                    <h1 class="display-5 fw-bold mb-2">📖 Vocabulary HSK {{ $level }}</h1>
                    <p class="text-muted mb-0">{{ $totalWords }} Words (Simplified Chinese)</p>
                </div>
            </section>
            ...
            <div class="mode-buttons-section">
                <div class="mode-buttons-grid">
                    <div class="mode-button" onclick="alert('Practice mode coming soon!')">
                        <span class="icon">📝</span>
                        <span>Practice</span>
                    </div>
                    <div class="mode-button" onclick="alert('Writing practice mode coming soon!')">
                    ...
                    <div class="mode-button"
                        onclick="window.location.href='{{ route('client.hsk.level', ['level' => $level]) }}?saved=1'">
                        <span class="icon">📖</span>
                        <span>Notebook</span>
                    </div>
                </div>
            ...
            <div class="autoplay-controls">
                <button class="autoplay-btn" id="autoplayBtn" onclick="toggleAutoplay()">
                    ...
                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 16 16" id="pauseIcon"
                        style="display:none;">
                        <path
                ...
                <span class="autoplay-progress"
                    id="autoplayProgress">{{ ($currentPage - 1) * $perPage + 1 }}/{{ $totalWords }}</span>
            </div>
                ...
                @foreach ($words as $index => $word)
                    <div class="vocab-card-item" data-word="{{ json_encode($word) }}" data-index="{{ $index }}">
                        ...
                        <div class="card-image">
                            <img src="https://work.lehutv04.xyz/{{ md5($word['w']) }}_h.jpg" alt="{{ $word['w'] }}"
                        ...
                        <div class="card-body">
                            ...
                            <div class="card-actions">
                                <button class="action-btn learned"
                                    onclick="markAsLearned({{ ($currentPage - 1) * $perPage + $index }})"
                                ...
                                <button class="action-btn details" onclick="showDetails({{ $index }})"
                                    title="View details">
                                    <svg width="14" height="14" fill="currentColor" viewBox="0 0 16 16">
            ...
            <div class="pagination-controls">
                <div class="pagination-info">
                    ...
                    ({{ ($currentPage - 1) * $perPage + 1 }}-{{ min($currentPage * $perPage, $totalWords) }} of
                    {{ $totalWords }} words)
                </div>
                <div class="pagination-nav">
                    @if ($currentPage > 1)
                        <a href="{{ route('client.hsk.level', ['level' => $level, 'page' => $currentPage - 1]) }}"
                            class="page-btn">
                            <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                <path fill-rule="evenodd"
    ...
    <script>
        ...
        const level = '{{ $level }}';
        let learnedWords = JSON.parse(localStorage.getItem('hsk_' + level + '_learned') || '[]');
        ...
        function markAsLearned(globalIndex) {
            ...
            localStorage.setItem('hsk_' + level + '_learned', JSON.stringify(learnedWords));
        }
        ...
        <div class="row">
            <div class="col-md-6 text-center mb-4">
                <h2 class="display-3 fw-bold" style="color: #1976d2;">${word.w}</h2>
    ...
    `;
        ...
        function playAudio(text) {
            // Use Web Speech API for Simplified Chinese
        ...
        function startAutoplay() {
            ...
            autoplayInterval = setInterval(() => {
                ...
                if (currentAutoplayIndex >= words.length) {
                    ...
                    const nextPageLink = document.querySelector('.pagination-nav a[href*="page=' + (currentPage +
                        1) + '"]');
                    if (nextPageLink) {
        ...
        function playCurrentWord() {
            if (currentAutoplayIndex < words.length) {
                ...
                if (cards[currentAutoplayIndex]) {
                    cards[currentAutoplayIndex].scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    });
                    // Highlight current card
                    cards.forEach(c => c.style.transform = '');
                    cards[currentAutoplayIndex].style.transform = 'scale(1.05)';
                }
